...
#include "flow_export_program_info.h"
...
static const ebpf_program_info_t* _program_information_array[] = {&_ebpf_flow_program_info};

static std::vector<ebpf_program_section_info_with_count_t> _section_information = {
    {&_ebpf_flow_section_info[0], 1},
};
...
clear_ebpf_store()
{
    ...
    std::cout << "Clearing eBPF store (flow)" << std::endl;
    for (const auto& section : _section_information) {
